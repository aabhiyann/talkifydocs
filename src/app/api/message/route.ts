...
export const POST = async (req: NextRequest) => {
  try {
    ...
    const { fileId, message } = validateSendMessage(body);
    ...
    const results = await vectorStore.similaritySearch(sanitizedMessage, 4);
    ...
    const response_stream = new StreamingTextResponse(stream);
    return applySecurityHeaders(response_stream);
  } catch (error) {
